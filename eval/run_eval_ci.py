"""Run DeepEval prompt evaluations in CI using Gemini as the base LLM and judge.
...
- Uses Gemini (OpenAI-compatible endpoint) as the base LLM for generation.
- Uses Gemini as the DeepEval judge model.
...
def _build_config(args) -> Config:
    ...
    if not api_key:
        raise RuntimeError("GEMINI_API_KEY is required in CI to run evals.")
    return Config(
        path=args.config,
...
def _build_metrics() -> list:
    api_key = os.environ.get("GEMINI_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("GEMINI_API_KEY is required in CI to run judge metrics.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run DeepEval prompt evaluations in CI (Gemini base LLM).")
